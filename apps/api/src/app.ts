 ...
 * import it and use `app.inject()` without binding a port or starting
 * Socket.IO - the emit helpers in realtime/io.ts are no-ops until initIO runs.
 */
...
export async function buildApp({ testing = false }: BuildOptions = {}): Promise<FastifyInstance> {
  const app = Fastify({
    logger: testing ? false : isProd ? true : { transport: { target: "pino-pretty" } },
    trustProxy: true,
  });
  ...
  // a shared 300/min budget would make a full test run flaky, so tests opt out
  if (!testing) await app.register(rateLimit, { max: 300, timeWindow: "1 minute" });
